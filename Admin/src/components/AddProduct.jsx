...
const AddProduct = () => {
  ...
  return (
    <div className='p-8 box-border bg-white w-full rounded-sm mt-4 lg:m-7 '>
      ...
      <div className='mb-3 flx items-center gap-x-4'>
        <label htmlFor="file-input" className='cursor-pointer'>
          <img src={image?URL.createObjectURL(image):upload_area} alt="Upload" className='w-20 h-20 rounded-sm inline-block' />
        </label>
        <input onChange={imageHandler} type="file" id="file-input" hidden className='bg-primary max-w-80 w-full py-3 px-4' />
      </div>
      <button onClick={() => Add_Product()}className='btn_dark_rounded mt-4 flexCenter gap-x-1'><MdAdd /> Add Prouct</button>
    </div>
  );
}
