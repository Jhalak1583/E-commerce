...
const {getTotalCartAmount, all_products, cartItems, removeFromCart} = useContext(ShopContext);
  ...
  return (
    <section className='max_padd_container pt-28'>
                ...
                {all_products.map((e) => {
                    if(cartItems[e.id] > 0 ) {
                        return <tr key={e.id} className='border-b border-slate-900/20 text-gray-50 p-6 medium-14 text-center'>
                            <td className='flexCenter'><img src={e.image} alt="ProdImg" height ={43} width={43} className='rounded-lg ring-1 ring-slate-900/5 my-1'/></td>
                            <td><div className='line-clam-3'>{e.name}</div></td>
                            <td>${e.new_price}</td>
        ...
        {/* Cart details */}
        <div className='flex flex-col gap-20 my-16 p-8 md:flex-row ounded-md bg-white -full max-w-[666px]'>
            <div className='flex flex-col gap-10'>
                ...
                <button className='btn_dark_rounded w-44' >Ceckout</button>
                <div className='flex flex-col gap-10'>
