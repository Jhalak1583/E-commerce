...
import { MdClass, MdClose, MdMenu } from "react-icons/md";
  ...
  return (
    <header className="fixed top-0 left-0 w-full bg-white ring-1 ring-slate-900/5 z-10">
      <div className="px-4 flexBetween py-3 max-xs:px-2">
        ...
        {/* Buttons */}
        <div className="flexBetween gap-x-4 sm:gap-x-6 bold-16 ring-slate-900/30 rounded-full">
          {!menuOpened ? (
            <MdMenu onClick={toggleMenu} className="md:hidden cursor-pointer hover:text-secondary ring-slate-900/30 rounded-full" />
          ) : (
            <MdClose onClick={toggleMenu} className="md:hidden cursor-pointer hover:text-secondary ring-slate-900/30 rounded-full" />
          )}
          ...
          <FaOpencart className="p-1 h-8 w-8 ring-slate-900/30 ring-1 rounded-full" />
             ...
             {localStorage.getItem('auth-token') ? <NavLink onClick={() => {localStorage.removeItem('auth-token'); window.location.replace("/")}} to="logout" className="btn_secondary_rounded flexCenter gap-x-1 medium-16">
